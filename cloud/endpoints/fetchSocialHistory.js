    ...
    this.fetchSocialHistory = function fetchSocialHistory(reqJson, callback){
      if (jsonUtils.getPath(reqJson, "request.head.sessionId") == null)         
        ...
        sessionManager.getSession(sessionId, function(err, data ){
            ...
            if(data)
            {
                //Setting the apiSession to fetch the socialHistory details      
                ...
                var optionsGet = {
                    ...
                    path : appConfig.environments[env].urls.socialHistory,
                    method : 'GET',
                    headers : getHeaders
                };
                ...
                var reqGet = http.request(optionsGet, function(res) {
                    ...
                    {
                         var fail = respUtils.constructStatusResponse("fetchSocialHistory", constants.RESP_AUTH_FAILED, "Authentication  Fail",{});
                        return  callback(fail,null) 
                    }
                    else if (res.statusCode == 500)
                    {
                        var fail = respUtils.constructStatusResponse("fetchSocialHistory", constants.RESP_SERVER_ERROR, "Internal server error",{});
                        return  callback(fail,null) 
                    }
                    ...
                    {                    
                        ...
                        res.on('end',function(){
                            if(data)
                            {
                                ...
                                var jsonObj = respUtils.constructStatusResponse("fetchSocialHistory", constants.RESP_SUCCESS, "fetchSocialHistory Success",jsonObject);
                ...
                reqGet.on('error', function(e) {
                    log.error("[fetchSocialHistoryEndpoint][fetchSocialHistory][regGet] >> " + e);
                    return  callback(fail,null)
                });
            } 
